...
| **Local LLMs** | TinyLlama, llama.cpp, GGUF quantized models |
...
- [x] **Day 32:** 4-Bit Quantization - Run a GGUF TinyLlama model locally with llama.cpp and reduced memory usage.
...
Day 32 is complete. Phase 4 now includes local Hugging Face generation plus 4-bit GGUF inference with llama.cpp, showing how quantization reduces memory needs for laptop-friendly LLMs. The project now has:
...
- `day32_quantization.py` for downloading a 4-bit TinyLlama GGUF model, loading it with llama.cpp, and generating local text with reduced memory footprint.
...
- `requirements.txt` with the Day 1 through Day 32 Python dependencies.
...
### 🧩 4-Bit Quantized LLM Inference (`day32_quantization.py`)

Downloads the 4-bit `TinyLlama-1.1B-Chat` GGUF model, loads it with `llama.cpp`, offloads inference to Apple Silicon GPU layers, and demonstrates why quantization helps local language models fit into laptop memory.

```bash
python day32_quantization.py
# --- Day 32: 4-Bit Quantization (GGUF & Llama.cpp) ---
#
# Downloading/Verifying 4-bit GGUF model:
# tinyllama-1.1b-chat-v1.0.Q4_K_M.gguf
# Loading Quantized Model into RAM...
# User Prompt:
# Why does quantizing model weights to 4 bits help run language models on a laptop?
#
# AI Response:
# Quantizing model weights to 4 bits helps run language models on a laptop by reducing
# the number of bits required to represent the weights of the model.
# Generation Speed: 0.30 seconds
```
...
# 2. Install Day 1 through Day 32 dependencies
...
# 34. Run 4-bit GGUF quantized local inference
python day32_quantization.py
