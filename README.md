...
### 🧮 LoRA Parameter Math (`day51_lora_math.py`)

Introduces parameter-efficient fine-tuning by comparing a full dense LLM layer with a rank-8 LoRA adapter. The script shows how freezing the base model and training only small adapter matrices dramatically reduces trainable parameters.

```bash
python day51_lora_math.py
# --- Day 51: The Math Behind LoRA (Low-Rank Adaptation) ---
#
# Standard Layer Parameters (Full Fine-Tuning): 16,781,312
# LoRA Adapter Parameters (r=8): 65,536
# PARAMETER REDUCTION: We are only training 0.39% of the weights!
```
