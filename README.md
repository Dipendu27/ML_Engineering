...
- [x] **Day 39:** Hardware Acceleration - Profile Apple Silicon Metal memory during quantized MLX inference.
- [x] **Day 40:** Inference Optimization - Tune streaming inference with sampler controls, top-p sampling, and repetition penalties.

## ⚙️ Phase 5: Building Agentic RAG (Days 41–50)

- [x] **Day 41:** Document Ingestion - Load clinical guidelines into LangChain `Document` objects with source metadata.
- [x] **Day 42:** PDF Parsing - Generate and parse a multi-page clinical discharge PDF with LangChain page metadata.
- [x] **Day 43:** Chunking - Split clinical PDFs into overlapping semantic chunks while preserving page metadata.
- [x] **Day 44:** Embedding Generation - Batch vectorize clinical chunks with BGE-Small through LangChain Hugging Face embeddings.
- [x] **Day 45:** Vector Storage Setup - Initialize a persistent LangChain Chroma collection for clinical RAG records.
- [x] **Day 46:** Populating ChromaDB - Insert clinical PDF chunks with deterministic IDs to prevent duplicate records.
- [x] **Day 47:** Semantic Search - Query ChromaDB for nearest clinical chunks with citation metadata and distance scores.
- [x] **Day 48:** Orchestration Logic - Format retrieved chunks into a citation-aware Llama-ready RAG prompt.
- [x] **Day 49:** LLM Integration - Feed retrieved Chroma context into a local MLX LLM for cited answer generation.
- [x] **Day 50:** RAG Fusion - Build an interactive production-style local clinical RAG assistant.
...
Day 50 is complete. Phase 5 is now complete and includes LangChain document ingestion, multi-page clinical PDF parsing, overlapping semantic chunking, batch embedding generation, persistent Chroma vector-store setup, secure chunk insertion, semantic nearest-neighbor retrieval, prompt orchestration, local MLX LLM answer generation, and an interactive local clinical RAG assistant. The project now includes local Hugging Face text generation, YOLOv8 object detection on Apple Silicon, model-specific prompt template formatting, Apple MLX lazy-evaluation optimization, native MLX-LM generation, local 4-bit model quantization, pre-quantized Llama-3 inference with MLX-LM, Apple Silicon Metal memory profiling, tuned streaming inference controls, LangChain clinical document loading, PDF page-level metadata extraction, chunk-level citation preservation, BGE-Small vector generation, a local persistent RAG database backend, deterministic Chroma record IDs, citation-aware semantic search results, Llama-ready RAG prompt payloads, streamed local RAG answers, and a reusable `LocalRAGAssistant` class. The project now has:
...
- `day39_hardware_profiling.py` for measuring MLX Metal idle memory, peak memory, KV cache allocation, and generation time during local inference.
- `day40_inference_optimization.py` for tuning local streaming inference with temperature, top-p sampling, repetition penalties, and context-window trimming.
- `day41_langchain_ingestion.py` for loading a clinical guideline text file into LangChain `Document` objects with source metadata.
- `day42_pdf_parsing.py` for generating a sample discharge PDF and parsing it into page-level LangChain `Document` objects.
- `day43_semantic_chunking.py` for splitting parsed clinical PDF pages into overlapping chunks while preserving source and page metadata.
- `day44_embedding_generation.py` for batch-generating normalized BGE-Small embedding vectors from parsed clinical chunks.
- `day45_vector_storage.py` for initializing a persistent LangChain Chroma collection backed by the BGE-Small embedding function.
- `day46_populate_database.py` for inserting chunked clinical PDF records into ChromaDB with deterministic IDs for duplicate-safe ingestion.
- `day47_semantic_search.py` for querying the populated ChromaDB collection and returning nearest clinical chunks with distance scores and citation metadata.
- `day48_orchestration_logic.py` for formatting retrieved Chroma chunks into a citation-aware Llama-style RAG prompt payload.
...
- `requirements.txt` with the Day 1 through Day 50 Python dependencies.
...
### 📈 Apple Silicon Hardware Profiling (`day39_hardware_profiling.py`)

Profiles a quantized MLX model during local text generation by measuring idle Metal memory, peak memory, KV cache allocation, and total generation time. This shows why 4-bit models fit comfortably inside a 16GB Apple Silicon workflow.

```bash
python day39_hardware_profiling.py
# --- Day 39: M5 Neural Engine & Hardware Profiling ---
#
# Loading mlx-community/Meta-Llama-3-8B-Instruct-4bit...
# Metal VRAM Usage (Idle): ...
# Generating text and stressing the M5 GPU...
#
# APPLE SILICON (METAL) PROFILING REPORT:
# Total Generation Time: ...
# Idle Model Footprint: ...
# KV Cache Allocation: ...
# Peak VRAM Spiked To: ...
```
...
### 🎛️ Advanced Inference Optimization (`day40_inference_optimization.py`)

Builds on the streaming chat loop by adding reusable MLX sampler controls for temperature and top-p sampling, plus a repetition penalty logits processor. This makes local responses more controlled while preserving the same context-window trimming used in the previous chat assistant.

```bash
python day40_inference_optimization.py
# --- Day 40: Advanced Inference Optimization (Final) ---
#
# Loading mlx-community/Meta-Llama-3-8B-Instruct-4bit into Unified Memory...
...
### 📥 LangChain Document Ingestion (`day41_langchain_ingestion.py`)

Starts the agentic RAG phase by loading a local clinical guideline text file with LangChain's `TextLoader`. The output is a standardized `Document` object containing both raw page content and metadata, including the source filename for future vector database citations.

```bash
python day41_langchain_ingestion.py
# --- Day 41: LangChain Document Ingestion Pipeline ---
#
# Initializing LangChain TextLoader...
# Ingesting data into memory...
# Successfully ingested 1 document(s).
# METADATA:
...
### 📄 Clinical PDF Parsing (`day42_pdf_parsing.py`)

Generates a small two-page discharge summary PDF with `FPDF`, then parses it with LangChain's `PyPDFLoader`. Each parsed page becomes a separate `Document` with source and page metadata, which is exactly the citation structure needed before chunking and vector storage.

```bash
python day42_pdf_parsing.py
# --- Day 42: Parsing Clinical PDFs with LangChain ---
#
# Loading 'patient_discharge.pdf' into the LangChain Pipeline...
# Successfully extracted 2 pages from the PDF.
# PAGE 1 METADATA:
# {'source': 'patient_discharge.pdf', 'page': 0, ...}
# PAGE 2 METADATA:
# {'source': 'patient_discharge.pdf', 'page': 1, ...}
```

---

### 🧩 Semantic Document Chunking (`day43_semantic_chunking.py`)

Loads the Day 42 discharge PDF and uses LangChain's `RecursiveCharacterTextSplitter` to split each page into small overlapping chunks. The overlap protects sentence-level context across boundaries, while LangChain copies the original `source` and `page` metadata into every child chunk for later citations.

```bash
python day43_semantic_chunking.py
# --- Day 43: Semantic Document Chunking & Overlap ---
#
# Ingesting PDF Document...
# Loaded 2 raw pages.
# Initializing Recursive Splitter (Size: 100, Overlap: 20)...
# Splitting Complete! The 2 pages became 7 chunks.
# CHUNK 1 | Source: patient_discharge.pdf (Page 0)
```

---

### 🧠 Batch Embedding Generation (`day44_embedding_generation.py`)

Reuses the Day 42 PDF parser and Day 43 chunking pipeline, then sends all chunk texts through LangChain's Hugging Face embedding wrapper with `BAAI/bge-small-en-v1.5`. The resulting normalized 384-dimensional vectors are ready for vector database storage and cosine-similarity retrieval.

```bash
python day44_embedding_generation.py
# --- Day 44: LangChain Batch Embedding Generation ---
#
# Ingesting and Chunking PDF...
# Created 7 semantic chunks.
# Loading Embedding Model: 'BAAI/bge-small-en-v1.5'...
# Successfully converted 7 chunks into math.
# Vector Dimensions: 384 numbers
```

---

### 🗄️ Persistent Vector Storage (`day45_vector_storage.py`)

Initializes a persistent LangChain Chroma vector store for the clinical RAG pipeline. The database is bound to the same normalized BGE-Small embedding function used on Day 44, so future text inserts and queries use a consistent vector space.

```bash
python day45_vector_storage.py
# --- Day 45: Initializing the Persistent Vector Database ---
#
# Loading Embedding Engine: 'BAAI/bge-small-en-v1.5'...
# Storage Target: clinical_rag_db
# Initializing ChromaDB Storage Backend...
# Database Setup Complete!
# Total documents currently in storage: 0
```

---

### 📦 Populating ChromaDB (`day46_populate_database.py`)

Runs the full ingestion path for the sample discharge PDF: load pages, split them into semantic chunks, embed with normalized BGE-Small vectors, and write the chunks into the persistent `clinical_rag_db` Chroma collection. Each chunk gets a deterministic ID based on source, page, and chunk index so reruns update existing records instead of creating duplicates.

```bash
python day46_populate_database.py
# --- Day 46: Populating ChromaDB (RAG Ingestion Pipeline) ---
#
# Loading and chunking clinical PDF...
# Generated 3 semantic chunks.
# Initializing BGE-Small Embedding Engine...
# Connecting to ChromaDB Storage...
# SUCCESS! Total chunks currently in ChromaDB: 3
```

---

### 🔎 Semantic Vector Search (`day47_semantic_search.py`)

Queries the populated `clinical_rag_db` Chroma collection with a natural-language clinical question, embeds the query with the same normalized BGE-Small model used during ingestion, and returns the nearest chunks with distance scores plus source/page metadata for citations.

```bash
python day47_semantic_search.py
# --- Day 47: Semantic Search & Vector Retrieval ---
#
# Doctor's Query: 'What is the recommended treatment for a patient with acute pericarditis?'
# Searching for the top 3 most relevant clinical chunks...
# MATCH #3 (Distance Score: 0.6985)
# Citation: patient_discharge.pdf | Page: 0
# Content: Treatment: Administered NSAIDs and Colchicine...
```

---

### 🧭 RAG Orchestration Logic (`day48_orchestration_logic.py`)

Wraps the retrieval step into an LLM-ready payload: Chroma returns the top clinical chunks, the script formats them with source/page citations, and the final prompt instructs the local model to answer only from the injected context.

```bash
python day48_orchestration_logic.py
# --- Day 48: RAG Orchestration & Prompt Injection ---
#
# Step A: Retrieving documents from ChromaDB...
# Step B: Formatting raw chunks into a single context string...
# Step C: Injecting Context and Query into the Master Prompt...
# FINAL PAYLOAD (The text we will send to Llama-3 tomorrow):
# [Document 3 | Source: patient_discharge.pdf | Page: 0]
# Treatment: Administered NSAIDs and Colchicine...
```

---

### 🧠 MLX LLM RAG Integration (`day49_rag_llm_integration.py`)

Connects the retrieval pipeline to a local MLX language model. The script retrieves the top clinical chunks from ChromaDB, injects them into a strict citation-aware prompt, and streams a local answer from `mlx-community/Meta-Llama-3-8B-Instruct-4bit` with low-temperature decoding.

```bash
python day49_rag_llm_integration.py
# --- Day 49: End-to-End RAG (Retrieval-Augmented Generation) ---
#
# Step A: Retrieving semantic chunks from ChromaDB...
# Step B: Injecting context into Llama-3 Prompt Template...
# Step C: Generating Answer (M5 Neural Engine)...
# AI: The recommended treatment is NSAIDs and Colchicine.
# Source: patient_discharge.pdf | Page: 0
```

---

### 🏥 Production Local RAG Assistant (`day50_rag_assistant.py`)

Packages the complete RAG workflow into a reusable `LocalRAGAssistant` class. The assistant initializes ChromaDB, BGE-Small retrieval, and the local MLX LLM once, then accepts repeated clinical questions in a terminal loop and streams cited answers from local context.

```bash
python day50_rag_assistant.py
# --- Day 50: Production-Ready Local RAG Framework ---
#
# Local Clinical Assistant Active. Ask your questions below.
# Enter Question: What is the recommended treatment for acute pericarditis?
# AI: The recommended treatment is NSAIDs and Colchicine.
# Source: patient_discharge.pdf | Page: 0
```
...
# 2. Install Day 1 through Day 50 dependencies
...
# 41. Run Apple Silicon Metal hardware profiling
python day39_hardware_profiling.py

# 42. Run optimized local streaming inference
python day40_inference_optimization.py

# 43. Run LangChain clinical document ingestion
python day41_langchain_ingestion.py

# 44. Run LangChain clinical PDF parsing
python day42_pdf_parsing.py

# 45. Run semantic document chunking
python day43_semantic_chunking.py

# 46. Run batch embedding generation
python day44_embedding_generation.py

# 47. Initialize persistent clinical RAG vector storage
python day45_vector_storage.py

# 48. Populate ChromaDB with clinical PDF chunks
python day46_populate_database.py

# 49. Run semantic vector search over ChromaDB
python day47_semantic_search.py

# 50. Build the citation-aware RAG prompt payload
python day48_orchestration_logic.py

# 51. Generate a local MLX LLM answer from RAG context
python day49_rag_llm_integration.py

# 52. Run the interactive local clinical RAG assistant
python day50_rag_assistant.py
